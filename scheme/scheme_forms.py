...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        env.bindings[signature.first]=do_lambda_form(Pair(expressions.first.rest, expressions.rest),env)
...
def do_and_form(expressions, env):
    ...
    temppair=expressions
    while(temppair.rest!=nil):
        if(is_scheme_false(scheme_eval(temppair.first,env))):
            return False
        temppair=temppair.rest
    return scheme_eval(temppair.first,env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if(is_scheme_true(scheme_eval(expressions.first,env))): return scheme_eval(expressions.first,env)
    else: return do_or_form(expressions.rest,env)
    # END PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    tempbind=bindings
    while(tempbind!=nil):
        validate_form(tempbind.first,2,2)
        names=Pair(tempbind.first.first,names)
        vals=Pair(scheme_eval(tempbind.first.rest.first,env),vals)
        tempbind=tempbind.rest
